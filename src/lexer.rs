...
pub fn lex(code: &str) {
  unsafe {
      ...
      TOKENS.push(match c {
        ...
        '=' => TokenTypes::Set,
        '?' => {code_iter.next(); TokenTypes::Equals},
        '>' => match code_iter.peek().unwrap() {
          '=' => {code_iter.next(); TokenTypes::GreaterThanOrEqualTo},
          _ => TokenTypes::GreaterThan
        }
        '<' => match code_iter.peek().unwrap() {
          '=' => {code_iter.next(); TokenTypes::LessThanOrEqualTo},
          _ => TokenTypes::LessThan,
        }
        '!' => {code_iter.next(); TokenTypes::NotEquals},
        '#' => {
        ...
        '"' | '\'' => {
          ...
          code_iter.next();
          TokenTypes::String(if accumulator.len() > 1 {accumulator} else {"".to_string()})
        },
        ...
        _ => {
          ...
          while !['=', '?', '!', '>', '<', ',', '(', ')', '{', '}', '[', ']', ' ', '\n'].contains(&namey) {
            ...
            if code_iter.peek().is_none() {break}
